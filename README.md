...
| 文件 | 覆盖内容 |
| :-- | :-- |
| `markdown-inline.test.ts` | 颜色 / 高亮 / 注音 / 剧透 / 容器等行内语法，含非法输入的安全降级 |
| `markdown-table.test.ts` | `[^] [v] [<] [>]` 四向合并、非矩形容错、列号计算 |
| `data-integrity.test.ts` | 71 个角色 JSON 的内容校验（战法图标、语音表结构、译名一致性） |

`data-integrity` 里的用例大多是**回归测试**，每条对应一次真实修复：

- 台词被 ruby 注音拆碎（`燈`/`阿呆`/`舁` 等）
- 分节标签混进台词单元格（tajima 的 Sleep 小节）
- 原 wiki 的编辑占位符残留
- 战法缺图标 / 遠江整页数据缺失
- `motifZh` 用裸粗体冒充标题

改数据或改解释器后跑一次 `npm test`，这些坑就不会重现。
